...
integration_qk :: 
 ...
 ->  Double     -- ^ b
 ->  GKStruct
integration_qk n xgk wg wgk f a b = gsk
  where
    center          = 0.5 * (a + b)
    half_length     = 0.5 * (b - a)
    abs_half_length = abs (half_length)
...
qk_15  :: 
 ...
 ->  Double     -- ^ b
 ->  GKStruct
qk_15 f a b = integration_qk 8 xgk15 wg15 wgk15  f a b
...
qk_21  :: 
 ...
 ->  Double     -- ^ b
 ->  GKStruct
qk_21 f a b = integration_qk 11 xgk21 wg21 wgk21 f a b

xgk15 = [  0.991455371120812639206854697526329,
           ...
           0.000000000000000000000000000000000 ] :: [Double]

-- xgk[1], xgk[3], ... abscissae of the 7-point gauss rule. 
--   xgk[0], xgk[2], ... abscissae to optimally extend the 7-point gauss rule */

-- /* weights of the 7-point gauss rule */
wg15 = [  0.129484966168869693270611432679082,
          0.279705391489276667901467771423780,
          0.381830050505118944950369775488975,
          0.417959183673469387755102040816327 ] :: [Double]

-- /* weights of the 15-point kronrod rule */
wgk15 = [  0.022935322010529224963732008058970,
           ...
           0.209482141084727828012999174891714 ] :: [Double]


 -- /* abscissae of the 21-point kronrod rule */
xgk21 = [  0.995657163025808080735527280689003,
...
wg21 = [  0.066671344308688137593568809893332,
          0.149451349150580593145776339657697,
          0.219086362515982043995534934228163,
          0.269266719309996355091226921569469,
          0.295524224714752870173892994651338] :: [Double]

--  /* weights of the 21-point kronrod rule */
wgk21 = [ 0.011694638867371874278064396062192,
          ...
          0.142775938577060080797094273138717,
          0.147739104901338491374841515972068,
          0.149445554002916905664936468389821 ] :: [Double]
